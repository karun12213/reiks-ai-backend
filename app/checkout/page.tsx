...
function CheckoutContent() {
    ...
    const handleConfirm = async () => {
        ...
        if (mode === 'buy') {
            ...
            const existingOrders = JSON.parse(localStorage.getItem('aureum_orders') || '[]')
            localStorage.setItem('aureum_orders', JSON.stringify([newOrder, ...existingOrders]))

            // Deduct applied SIP
            if (applySip && sipBalance > 0) {
                const localSip = JSON.parse(localStorage.getItem('aureum_sip') || '{}')
                if (localSip) {
                    ...
                    localStorage.setItem('aureum_sip', JSON.stringify(localSip))
                }
            }
            ...
            const existingLocks = JSON.parse(localStorage.getItem('aureum_locks') || '[]')
            localStorage.setItem('aureum_locks', JSON.stringify([newLock, ...existingLocks]))
        }
    ...
    if (isSuccess && mode === 'lock') {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
                <div className="bg-aureum-card border border-aureum-border p-8 rounded-2xl relative overflow-hidden max-w-sm w-full mx-auto shadow-[0_0_30px_rgba(234,179,8,0.05)]">
                    ...
                    <h2 className="text-xl font-heading font-bold text-aureum-white mb-2 relative z-10">
                        Price Locked Setup
                    </h2>
                    <p className="text-sm text-aureum-dim mb-6 relative z-10">
    ...
    return (
        <div className="max-w-3xl mx-auto px-4 py-8 md:py-16">
            <button onClick={() => router.back()} className="text-xs text-aureum-dim hover:text-gold flex items-center gap-1 mb-8 transition-colors">
            ...
            <div className="text-center mb-10">
                ...
                <p className="mt-2 text-sm text-aureum-dim flex items-center justify-center gap-2">
                    <Shield size={14} className="text-success" /> End-to-end encrypted
                </p>
            </div>

            <div className="bg-aureum-card rounded-2xl border border-aureum-border p-6 md:p-8">
                <div className="mb-8">
                    <h3 className="text-xs text-aureum-dim uppercase tracking-wider mb-4">Item Details</h3>
                    <div className="flex justify-between items-start pb-4 border-b border-aureum-border">
                        <div>
                            <div className="text-lg font-semibold text-aureum-white">{name}</div>
                            <div className="text-sm text-aureum-dim">{karat}K Gold • {weight}g</div>
                        </div>
                        <div className="text-right">
                            <div className="text-lg font-mono font-bold text-gold">{formatINR(price)}</div>
                            <div className="text-[10px] text-aureum-dim">incl. taxes</div>
                        </div>
                    </div>
                </div>
                ...
                {mode === 'lock' && (
                    <div className="mb-8">
                        <label className="text-xs text-aureum-dim uppercase tracking-wider mb-3 block">Lock Duration</label>
                        <div className="grid grid-cols-3 gap-3">
                            {(['24h', '48h', '7d'] as LockDuration[]).map(d => (
                                <button
                                    key={d}
                                    onClick={() => setDuration(d)}
                                    className={`py-3 rounded-lg text-sm font-mono text-center transition-all ${duration === d ? 'bg-gold text-aureum-black font-bold' : 'bg-aureum-dark border border-aureum-border text-aureum-mid hover:border-gold/30'}`}
                                >
                ...
                <div className="mb-8 p-4 bg-aureum-dark rounded-xl border border-aureum-border">
                    <div className="flex justify-between text-sm mb-2">
                        <span className="text-aureum-dim">{mode === 'buy' ? 'Order Total' : 'Total Locked Value'}</span>
                        <span className="text-aureum-white font-mono">{formatINR(price)}</span>
                    </div>
                    {mode === 'lock' && (
                        <div className="flex justify-between text-sm mb-2">
                            <span className="text-aureum-dim">Premium Required ({LOCK_PREMIUM_RATES[duration] * 100}%)</span>
                    ...
                    {sipBalance > 0 && mode === 'buy' && (
                        <div className="flex items-center justify-between py-3 my-2 border-y border-aureum-border/30">
                            <label onClick={(e) => {
                                if (sipBalance < price) {
                                    e.preventDefault()
                                    alert(`Insufficient SIP Balance. You need at least ₹${price.toLocaleString()} in your SIP to use this feature.`)
                                ...
                                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${applySip ? 'bg-gold border-gold' : 'border-aureum-dim group-hover:border-gold/50'}`}>
                                    {applySip && <Check size={14} className="text-aureum-black font-bold" />}
                                </div>
                                <div className="text-sm">
                                    <span className="text-aureum-white block">Apply Vault SIP Gold</span>
                                    <span className="text-[10px] text-aureum-dim">Balance: {formatINR(sipBalance)}</span>
                                </div>
                            </label>
                            {applySip && (
                    ...
                    <div className="flex justify-between text-base pt-3 mt-3 border-t border-aureum-border/50">
                        <span className="text-aureum-white font-medium">To Pay Now</span>
                ...
                <div className="space-y-4 mb-8">
                    <div>
                        <label className="text-xs text-aureum-dim mb-2 block">Card Details</label>
                        <div className="relative">
                            <input disabled placeholder="xxxx xxxx xxxx xxxx" className="w-full px-4 py-3 bg-aureum-dark border border-aureum-border rounded-lg text-aureum-dim cursor-not-allowed font-mono opacity-50" />
                            <CreditCard size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-aureum-dim" />
                        </div>
                        <p className="text-[10px] text-aureum-dim mt-2 group flex items-center gap-1">
